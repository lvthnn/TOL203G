...
\newcommand*\del[1]{\tikz[baseline=(char.base)]{
            \node[shape=circle,draw=red,inner sep=2pt] (char) {\color{red}\texttt{#1}};}}

\subsection*{Lausn}
Tafla fyrir neðan sýnir keyrsluna á Heapsort og hvernig \texttt{a[]} breytist. Einnig eru teikningar af hrúgunni
sem eru kannski meira lýsandi.
...
\begin{table}[H]
  ...
  \begin{tabular}{cccccccccc}
    \multicolumn{10}{*}{Bro?} //
    % \toprule
    % \texttt{N} & \texttt{k} & \texttt{0} & \texttt{1} & \texttt{2} & \texttt{3} & \texttt{4} & \texttt{5} & \texttt{6} & \texttt{7}
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  \centering
  \Tree[ .\circled{5} [ .\circled{4} [ .\circled{1} \circled{7} ] \circled{6} ] 
...
\begin{figure}[H]
  \centering
  \Tree[.\circled{1} [.\circled{3} [.\circled{5} \circled{8} ] \circled{5} ] 
                     [.\circled{4} \circled{6} \circled{7} ] ]
